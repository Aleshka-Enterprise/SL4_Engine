...
class FlappyBird(TimerMixin, AudioMixin, EventMixin):
    def __init__(self):
        ...
        BackgroundLayer(
            x=0, y=0, height=DISPLAY.HEIGHT, width=DISPLAY.WIDTH, state="startscreen", z_index=9999
        )
        self.event_listener = [
    ...
    def run(self) -> None:
        ...
        self.add_pipes_timer = self.add_timer(
            [self.generate_buildings], seconds=1.5, loop=True, use_on_start=True
        )
