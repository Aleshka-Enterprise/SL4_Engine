    ...
    for key, value in obj.__dict__.items():
        ...
        if isinstance(value, Rect):
            ...
            attributes[clean_key] = [
                (
                    serialize_obj(item, _seen)
                    if hasattr(item, "__dict__") or isinstance(item, Rect)
                    else item
                )
                for item in value
            ]
        ...
        for key, value in attributes.items():
            if isinstance(value, dict):
                if "__ref__" in value:
                    ...
                    processed_attrs[key] = created_objects.get(ref_id, value)
...
def generate_ai_documentation(registry):
    ...
    docs = {"version": "1.0", "classes": {}}

    for class_name, cls in registry.items():
        class_doc = {
            "description": cls.__doc__ or f"Class {class_name}",
            "parent": [base.__name__ for base in cls.__bases__ if not isinstance(base, object)],
            "init_parameters": {},
            "attributes": {},
            "required_attributes": [],
            "optional_attributes": [],
        ...
        try:
            ...
            obj = cls(**minimal_params) if cls(**minimal_params) else cls()
