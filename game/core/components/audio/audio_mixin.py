...
class AudioManager:
    ...
    def preload_sounds(cls, directory: str) -> None:
        ...
        for root, _dirt, files in os.walk(directory):
            for file in files:
                ...
                for action_name in os.listdir(audio_base_path):
                    ...
                    if os.path.isdir(action_path):
                        ...
                        if sound_group.sound_files:
                            ...
                            if DEBUG:
                                print(
                                    f"Загружена звуковая группа '{action_name}' для {class_name} с {len(sound_group.sound_files)} звуками"
                                )
    ...
    def get_next_sound(self) -> Optional[pygame.mixer.Sound]:
        ...
        if self.mode == PlaybackMode.RANDOM:
            ...
            if self.current_index >= len(self.sound_files):
                self.current_index = (
                    0 if self.mode == PlaybackMode.LOOP else len(self.sound_files) - 1
                )
...
class AudioMixin:
    ...
    def set_sound_group_variations(
        self,
        group_name: str,
        volume_range: Tuple[float, float] = (0.1, 1.2),
        variation_chance: float = 1.0,
    ) -> None:
    ...
    def play_sound(
        ...
        if group_name not in self._sound_groups:
            if DEBUG:
                print(
                    f"Звуковая группа '{group_name}' не найдена. Доступные: {list(self._sound_groups.keys())}"
                )
            return False
    ...
    def get_playing_sounds(self) -> List[str]:
        ...
        return [
            group_name
            for group_name, channel in self._currently_playing.items()
            if channel.get_busy()
        ]
