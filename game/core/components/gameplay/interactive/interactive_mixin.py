...
class InteractiveMixin(BaseMixin):
    ...
    def take_item(self, type: str = None, ignore=None) -> None:
        for item in (
            obj for obj in storage.render_objects_list if obj in storage.items and obj.can_pick
        ):
            if (
                self.__interactive_zone_rect.colliderect(item.rect)
                and (not type or type == item.item_type)
                and (not ignore or item not in ignore)
            ):
                return item
